...
const DateOverrideManagement = ({ 
  ...
  onBack,
  onUpdateDateOverrides,
  apiBaseUrl = 'http://localhost:8000/api/provider'
}) => {
  const [dateOverrides, setDateOverrides] = useState([]);
  const [showAddForm, setShowAddForm] = useState(false);
  const [editingOverride, setEditingOverride] = useState(null);
      ...
      <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4 flex items-start gap-2 sm:gap-3">
            ...
            <div className="text-xs sm:text-sm text-red-800 break-words">{error}</div>
          </div>
        )}
        ...
        {!showAddForm && (
          <button
            onClick={() => setShowAddForm(true)}
            className="w-full py-2.5 sm:py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-600 hover:border-gray-400 hover:text-gray-700 transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
          >
        ...
        {/* Add/Edit Override Form - Mobile Responsive */}
        {showAddForm && (
          <div className="bg-gray-50 rounded-lg p-3 sm:p-6 space-y-3 sm:space-y-4 border-2 border-green-100">
            ...
            <div>
              ...
              <input
                ...
                disabled={!!editingOverride}
                min={new Date().toISOString().split('T')[0]}
                className="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-gray-500 disabled:bg-gray-100 disabled:cursor-not-allowed text-sm sm:text-base"
              />
              {editingOverride && (
                <p className="mt-1 text-xs text-gray-500">Date cannot be changed when editing</p>
              )}
            ...
            {/* Close Entire Day Option */}
            <div className="flex items-start sm:items-center gap-3 p-3 sm:p-4 bg-white rounded-lg border border-gray-200">
              <input
                type="checkbox"
                id="closeDay"
                checked={newOverride.isClosed}
                ...
                className="h-4 w-4 text-gray-600 rounded focus:ring-gray-500 mt-0.5 sm:mt-0 flex-shrink-0"
              />
              <label htmlFor="closeDay" className="text-xs sm:text-sm font-medium text-gray-700">
                Close entire day (no appointments available)
              </label>
            </div>

            {/* Custom Hours - Mobile Responsive */}
            {!newOverride.isClosed && (
              <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <input
                        ...
                        className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 text-sm sm:text-base"
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      <label className="block text-xs text-gray-600 mb-1">End Time</label>
                ...
                {/* Blocked Time Slots - Mobile Responsive */}
                <div>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                    ...
                    <button
                      onClick={addBlockedTimeSlot}
                      className="text-xs sm:text-sm text-gray-600 hover:text-gray-800 flex items-center gap-1 self-start"
                    >
                  ...
                  {newOverride.blockedTimeSlots.map((slot, index) => (
                    <div key={index} className="bg-white p-2 sm:p-3 rounded-lg border border-gray-200 mb-2">
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-2">
                        <input
                          type="time"
                          value={slot.startTime}
                          onChange={(e) => updateBlockedTimeSlot(index, 'startTime', e.target.value)}
                          className="flex-1 px-2 py-1 border border-gray-300 rounded text-xs sm:text-sm min-w-0"
                          placeholder="Start"
                        />
                        <span className="text-gray-500 text-xs sm:text-sm self-center sm:self-auto">to</span>
                        <input
                          type="time"
                          value={slot.endTime}
                          onChange={(e) => updateBlockedTimeSlot(index, 'endTime', e.target.value)}
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-3 sm:pt-4">
              ...
              <button
                onClick={handleAddOverride}
                className="w-full sm:w-auto px-4 py-2 text-sm font-medium text-white bg-gray-600 rounded-lg hover:bg-gray-700"
              >
                {editingOverride ? 'Update Override' : 'Add Override'}
              </button>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            {sortedOverrides.length > 0 && (
              <button
                onClick={handleDeleteAllOverrides}
                disabled={isDeleting}
                className="inline-flex items-center justify-center sm:justify-start px-3 py-1.5 text-xs sm:text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isDeleting ? (
                  <>
                    <div className="animate-spin rounded-full h-3 h-3 sm:h-4 sm:w-4 border-b-2 border-red-600 mr-2"></div>
                    Deleting...
                  </>
                ) : (
                  <>
            ...
            sortedOverrides.map((override, index) => (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-sm sm:text-base text-gray-900 mb-1 break-words">
                      {formatDateDisplay(override.date)}
                    </div>
                    {override.isClosed ? (
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                      ...
                      <div className="space-y-1">
                        {override.customHours?.startTime && override.customHours?.endTime && (
                          <div className="flex items-center text-xs sm:text-sm text-gray-600">
                            ...
                            <span className="break-words">Custom Hours: {override.customHours.startTime} - {override.customHours.endTime}</span>
                          </div>
                        )}
                        {override.blockedTimeSlots?.length > 0 && (
                          <div className="text-xs sm:text-sm text-gray-600">
                            <div className="font-medium">Blocked Slots:</div>
                            {override.blockedTimeSlots.map((slot, i) => (
                              <div key={i} className="ml-4 break-words">
                                • {slot.startTime} - {slot.endTime} 
                                {slot.reason && <span className="text-gray-500"> ({slot.reason})</span>}
                              </div>
                            ))}
                        ...
                        {!override.customHours?.startTime && !override.blockedTimeSlots?.length && (
                          <span className="text-xs sm:text-sm text-gray-500 italic">Using regular schedule</span>
                        )}
                      </div>
                    )}
                  </div>
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-4 sm:pt-6 border-t border-gray-200">
          <button
            onClick={onBack}
            className="w-full sm:w-auto px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={isSaving || !hasUnsavedChanges}
            className="w-full sm:w-auto px-4 py-2 text-sm font-medium text-white bg-gray-600 rounded-lg hover:bg-gray-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center"
          >
            {isSaving ? (
              <>
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
