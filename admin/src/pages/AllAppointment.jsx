...
const AllAppointments = () => {
  ...
  useEffect(() => {
    // Only call once when component mounts and hasn't been loaded yet
    if (!hasInitialLoad && !loadingStates?.appointments) {
  ...
  // Helper function to normalize status - MOVED UP
  ...
  const getAppointmentStyling = useCallback((appointment) => {
    ...
    // Use provider color for pending/confirmed appointments
    return providerColor;
  }, [getAppointmentStatus, getProviderColor]);

  // Centralized handlers using context functions
...
const ActionButton = ({ color, icon, onClick, label, disabled, appointmentId, action }) => {
  ...
  const AppointmentCard = ({ appointment }) => {
    ...
    // Get service title - handle both array and direct service
  ...
  (appointment.consentForm.healthConditions || appointment.consentForm.allergies) && (
    <div className="mb-4">
      <div className="text-xs font-medium text-gray-500 mb-1">Consent Form</div>
      <div className="text-sm text-gray-700 bg-blue-50 p-2 rounded border-l-4 border-blue-200">
        {appointment.consentForm.healthConditions && (
          <div>Health Condition: {appointment.consentForm.healthConditions}</div>
        )}
        {appointment.consentForm.allergies && (
          <div>Allergies: {appointment.consentForm.allergies}</div>
        )}
      </div>
    </div>
  )
...
{/* // 2. Update the AppointmentCard action buttons section to be more responsive */}
{canTakeAction && (
  ...
  // Empty state (when not loading but no appointments)
...
//   // Helper function to normalize status - UPDATED TO HANDLE CONFIRMED STATUS
//   const getAppointmentStatus = useCallback((appointment) => {
...
//     // For appointments without explicit status, treat as pending for action purposes
//     // This allows confirmed appointments to show action buttons
//     return 'pending';
//   }, []);
...
//   // StatusBadge component - UPDATED TO SHOW PROPER STATUS
//   const StatusBadge = ({ appointment }) => {
//     const status = getAppointmentStatus(appointment);
...
//     // Show the actual status from the appointment, but if it's confirmed, show as confirmed
//     const displayStatus = appointment?.status?.toLowerCase() || status;
...
//                 const styling = getAppointmentStyling(apt);
//                 const status = getAppointmentStatus(apt);
//                 const canTakeAction = status === 'pending' || (!apt.isCompleted && !apt.isCancelled);
                
//                 return (
//                   <div
//                     key={apt._id || idx}
//                     className={`text-xs p-2 rounded-md border ${styling.bg} ${styling.text} ${styling.border} transition-colors relative overflow-hidden group`}
//                     title={`${apt.time} - ${apt.userName} - ${apt.serviceTitle} (${apt.providerName})`}
//                   >
//                     {/* Provider color accent bar */}
//                     <div className={`absolute left-0 top-0 bottom-0 w-1 ${styling.accent}`}></div>
...
//                       {/* Quick action buttons for calendar - only show on hover for larger screens */}
//                       {canTakeAction && (
...
//   const AppointmentCard = ({ appointment }) => {
//     const status = getAppointmentStatus(appointment);
//     // UPDATED: Allow actions for appointments that aren't completed or cancelled
//     // const canTakeAction = !appointment.isCompleted && !appointment.isCancelled;
//     const canTakeAction = !appointment.isCompleted && !appointment.isCancelled && loadingId !== appointment._id;
