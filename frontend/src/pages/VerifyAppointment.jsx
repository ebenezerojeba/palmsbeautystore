...
// GET METHOD
// FRONTEND - Updated for GET method
import { useContext, useEffect, useState } from 'react';
import { useParams, useSearchParams, useNavigate } from 'react-router-dom';
import axios from 'axios';
import { Loader2 } from 'lucide-react';
...
export default function VerifyAppointment() {
  ...
  const verifyPayment = async (attempts = 0) => {
    ...
    try {
      // Changed to GET request with appointmentId in URL and sessionId as query param
      const { data, status: httpStatus } = await axios.get(
        `${backendUrl}/api/appointment/verify/${appointmentId}?session_id=${sessionId}`,
        config
      );
          ...
          <div className="flex flex-col items-center gap-4 text-red-600">
            ...
            {debugInfo && (
              <div className="text-xs text-gray-500 mb-4 p-2 bg-gray-100 rounded">
                <p>Debug: Status {debugInfo.httpStatus}, Attempt {debugInfo.attempt}</p>
                <p>URL: {backendUrl}/api/appointment/verify/{appointmentId}</p>
              </div>
            )}
