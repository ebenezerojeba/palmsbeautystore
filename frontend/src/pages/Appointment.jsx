...
//         if (data.service) {
//           setServiceInfo(data.service);
//           setSelectedServices([data.service]); // Initialize with the primary service
//         } else {
//           toast.error("Service not found");
//           navigate("/services");
//         }
...
//       try {
//         const res = await fetch(`${backendUrl}/api/services/all-services`);
//         const data = await res.json();
//         if (data.services) {
...
//     return selectedServices.reduce((total, service) => total + (service.duration || 90), 0);
//   };
...
//   // Fetch available slots based on total duration
//   useEffect(() => {
//     const fetchAvailableSlots = async () => {
...
//         const res = await fetch(
//           `${backendUrl}/api/appointment/available-slots?serviceId=${id}&startDate=${startDate}&endDate=${endDate}&duration=${totalDuration}`
//         );
//         const data = await res.json();
...
//       // Reset date/time selection when services change
//       setSelectedDate("");
//       setSelectedTime("");
...
//                     <div key={service._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border">
...
//                       <p className="font-semibold text-blue-900">{formatDuration(getTotalDuration())}</p>
//                     </div>
//                     <div className="text-right">
//                       <p className="text-sm text-blue-600">Total Price</p>
//                       <p className="text-2xl font-bold text-blue-900">{formatNaira(getTotalPrice())}</p>
//                     </div>
//                   </div>
//                 </div>
//               </div>
...
//                     <div className="text-center py-8 text-gray-500 bg-gray-50 rounded-lg">
//                       <AlertCircle className="h-8 w-8 mx-auto mb-2 text-gray-400" />
//                       <p>No available slots for the selected services duration ({formatDuration(getTotalDuration())})</p>
//                     </div>
//                   )}
//                 </div>
...
//                     <>
//                       <Calendar className="h-5 w-5 mr-2" />
//                       Book Appointment - {formatNaira(getTotalPrice())}
//                     </>
//                   )}
//                 </button>
...
//                 <div className="mt-4 flex items-start text-sm text-gray-500">
//                   <Info className="h-4 w-4 mr-2 mt-0.5 text-blue-500 flex-shrink-0" />
//                   <p>Your appointment will be confirmed after payment is completed. Multiple services will be scheduled consecutively.</p>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//                             </div>
//                           </div>
//                           <span className="font-medium text-right">{formatNaira(service.price)}</span>
//                         </div>
//                       ))}
//                     </div>
...
//                   <h3 className="text-xl font-bold text-gray-900">Add Another Service</h3>
//                   <button
...
//                     .map((service) => (
//                     <div key={service._id} className="p-4 border rounded-lg hover:border-blue-300 transition-colors">
...
const Appointment = () => {
  ...
  //       // Extract numbers from strings like "90 minutes" or "1h 30m"
  //       const match = duration.match(/(\d+)/);
  //       duration = match ? parseInt(match[1]) : 90;
  //     }
  //     return total + (duration || 90);
  //   }, 0);
  // };
  ...
  useEffect(() => {
    const fetchAvailableSlots = async () => {
      ...
      try {
        ...
        // console.log('Fetching slots with duration:', totalDuration); // Debug log
        ...
        if (data.availableSlots) {
          ...
          console.error('No availableSlots in response:', data);
          setAvailableSlots([]);
        }
      } catch (err) {
              ...
              <div className="p-6">
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
                  <div className="flex justify-between items-center">
                    {/* <div>
                      <p className="text-sm text-blue-600">Total Duration</p>
                      <p className="font-semibold text-blue-900">{(getTotalDuration())}</p>
                    </div> */}
                    <div className="text-right">
                      <p className="text-sm text-blue-600">Total Price</p>
                      <p className="text-2xl font-bold text-blue-900">{formatNaira(getTotalPrice())}</p>
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="mb-8">
                  ...
                  {isLoading ? (
                    ...
                    <div className="text-center py-8 text-gray-500 bg-gray-50 rounded-lg">
                      <AlertCircle className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                      <p>No available slots for the selected services duration</p>
                      <p className="text-sm mt-1">Try selecting fewer services or different services</p>
                    </div>
                  )}
                </div>
